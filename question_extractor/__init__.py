...
from .markdown import files_from_directory, split_markdown
from .token_counting import count_tokens_text, are_tokens_available_for_both_conversations
...
async def extract_questions_from_text(file_path, text):
    ...
    if not are_tokens_available_for_both_conversations(num_tokens_text):
        ...
        tasks = []
        for sub_title, sub_text in await split_markdown(text):
...
def extract_questions_from_directory(input_folder, verbose=True):
    ...
    files = files_from_directory(input_folder)
